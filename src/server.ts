...
fastify.register(cors, { origin: '*', methods: ['GET', 'POST'] });
...
const startServer = async () => {
  try {
    ...
    await fastify.register(fastifyJwt, { secret: SECRET_KEY! });
    await fastify.ready();
    setupSocketIO();
    fastify.addHook('onRequest', jwtAuth);
    fastify.register(userRoutes, { prefix: '/user' });
    fastify.setNotFoundHandler((_request: FastifyRequest, reply: FastifyReply) =>
      responseHelper(reply, HTTP_STATUS_CODE.NOT_FOUND, 'Route not found'),
    );
