...
export const signIn = async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  if (!user) return responseHelper(reply, HTTP_STATUS_CODE.NOT_FOUND, 'User not found');
...
export const signUp = async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  const savedUser = await UserModel.create({ name, email, secret: secret.base32 });
  if (!savedUser) return responseHelper(reply, HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR, 'User not saved');
...
export const verifyTotp = async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  const user = await UserModel.findOne({ name, email });
  if (!user) return responseHelper(reply, HTTP_STATUS_CODE.NOT_FOUND, 'User not found');
  const isValidOtp = verifyTokenTotp(user.secret, otp);
  if (!isValidOtp) return responseHelper(reply, HTTP_STATUS_CODE.UNAUTHORIZED, 'Invalid TOTP');
